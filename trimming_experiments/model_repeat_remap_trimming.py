...
def get_posterior_inter(reads_1,reads_2,dist_prob,region_size,if_max=0):
    ...
    if(np.sum(inter_prob)==0):
        ...
        print(reads_1.maps_prob)
        print(reads_2.maps_prob)
        print("Attention 2: ")
        print(inter_prob.shape)
        print(inter_prob)
        '''
...
def mapping_back(input_path,input_file,output_path,align_num=2):
    ...
    '.bwt2glob.unmap.fastq --rg-id BMG --rg SM:merge --phred33-quals -p 10 -k ' + str(align_num) + ' -x /mnt/disk53/user/nsauerwa/HiCPro_stuff/bt2idx_hg19/hg19 -U '+\
...
def mapping_back_maize(input_path,input_file,output_path,align_num=2):
    file_name = input_file.split(".")[0]
    cmd = 'samtools bam2fq ' + input_path + input_file + ' > ' + output_path + file_name + '.fastq'
    os.system(cmd)
    cmd = 'bowtie2 --very-sensitive -L 30 --score-min L,-0.6,-0.2 --end-to-end --reorder --un '+output_path+file_name+\
    '.bwt2glob.unmap.fastq --rg-id BMG --rg SM:merge --phred33-quals -p 10 -k ' + str(align_num) + ' -x /mnt/disk69/user/yihangs/HiC_correction_revision/maize_data/Zea_mays/Ensembl/AGPv4/Sequence/Bowtie2Index/AGPv4 -U '+\
    output_path+file_name+'.fastq 2> '+output_path+'bowtie_'+file_name+'_global.log | samtools view -F 4 -bS - > '+output_path+file_name+'.bwt2glob.bam'
    os.system(cmd)
    return
